 ...
 * Followers list.
 *
 * Backend: GET `/follow/{userId}/followers`
 */
@Component({
  ...
  template: `
    <mat-card>
      ...
      <mat-card-content>
        ...
        <div *ngIf="!loading() && !users().length" class="hint">No followers found.</div>
      </mat-card-content>
...
export class FollowersPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.followApi.followers(this.userId).subscribe({
      next: (res) => {
        this.users.set(res);

        // Keep back link as provided via query param.

        this.loading.set(false);
      },
      error: (e) => {
        this.error.set(e?.error?.message ?? 'Failed to load followers');
        this.loading.set(false);
      }
    });
  }
